...
public class _BankService {
	...
	private static Map<String, _Bank> bankMap = new HashMap<>(); // number -> Bank
	...
	public static void main(String[] args) {
		...
		post("/banks", (req, res) -> {
			...
			_Bank bank = new _Bank(gameID, bankNumCounter++);
...
//			}
		...
		post("/banks/:number/accounts", (request, response) -> {
			...
			_Account account = gson.fromJson(request.body(), _Account.class);
			account.setBankURI(number);
			_Bank bank = bankMap.get(number);
		...
		post("/banks/:bankuri/transfer/to/:playerid/:amount", (request, response) -> {
			...
			if(accountMap.containsKey(playerid)) {
				...
				JSONObject json = new JSONObject(request.body());
				_TransactionService.getInstance().createTransaction(bankuri, json.getString("description"), playerid);
				response.status(HttpStatus.CREATED_201);
				// response.type("application/json");
		...
		post("/banks/:bankuri/transfer/from/:from/to/:to/:amount", (request, response) -> {
			...
			_Account accountFrom = null;
			_Account accountTo = null;
			if(accountMap.containsKey(fromPlayer) && accountMap.containsKey(toPlayer)) {
				...
				if(accountFrom.toBank(amount)) {
					...
					JSONObject json = new JSONObject(request.body());
					_TransactionService.getInstance().createTransaction(bankuri, json.getString("description"), fromPlayer);
					response.status(HttpStatus.CREATED_201);
					// response.type("application/json");
					// response.body("A new bank transfer has been created");
				} else {
